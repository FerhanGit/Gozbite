...
exit;
  ...
  if (isset($_REQUEST['ActionTripCtgr']) && ($_REQUEST['ActionTripCtgr'] == 1) && isset($_REQUEST['tcID']) && ($_REQUEST['tcID'] > 0)) {
      require("classes/TripCategory.class.php");

      $TripCtgr = new TripCategory($conn);
      $TripCtgr->id = $_REQUEST['tcID'];
      if (!$TripCtgr->deleteTripCtgr())
...
<input type = "hidden" name = "ActionTripCtgr">
  ...
  <div id="CENTER" style="margin-left:0px;">
     ...
     <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;ЕКСКУРЗИИ | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newPostType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_trip_category.php?tcID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/TripCategoriesList.class.php");
	            $CCList = new TripCategoriesList($conn);
