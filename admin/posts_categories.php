...
exit;
  ...
  if (isset($_REQUEST['ActionPostCtgr']) && ($_REQUEST['ActionPostCtgr'] == 1) && isset($_REQUEST['pcID']) && ($_REQUEST['pcID'] > 0)) {
       require("classes/PostCategory.class.php");

      $PostCtgr = new PostCategory($conn);
      $PostCtgr->id = $_REQUEST['pcID'];
      if (!$PostCtgr->deletePostCtgr())
...
<input type = "hidden" name = "ActionPostCtgr">
     ...
     <div id="BANER_KVADRAT_AND_NEWS"style="float:left; width:650px; margin-left:10px; margin-top:10px;background-color:#F9FFF9;">
      ...
      <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;Статии | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newPostType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_post_category.php?pcID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/PostCategoriesList.class.php");
