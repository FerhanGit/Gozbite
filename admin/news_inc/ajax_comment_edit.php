<?php
   ...
   $response = "";
        ...
        $sql="SELECT * FROM news_comment WHERE newsID='".$_REQUEST['all_newsID']."' ORDER BY created_on DESC";
		$conn->setsql($sql);
		$conn->getTableRows();
      ...
      if($numComment > 0) {
          for($k=0;$k<$numComment;$k++)
          {
                    ...
                    $response .= "  <input type='hidden' name='newsID[]' value=".$resultComment[$k]['newsID'].">";
