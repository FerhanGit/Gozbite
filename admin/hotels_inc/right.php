<?php
if(!empty($_REQUEST['category']) or !empty($_REQUEST['doctor_category']))
{
	$sql="SELECT postID,title FROM posts WHERE body LIKE '%".get_doctor_category($_REQUEST['category']?$_REQUEST['category']:$_REQUEST['doctor_category'])."%' OR title LIKE '%".get_doctor_category($_REQUEST['category']?$_REQUEST['category']:$_REQUEST['doctor_category'])."%'  ORDER BY RAND() DESC LIMIT 5";
}
else 
{
...
{
?>
  <div class="last_posts" style=" float:left; margin:5px; padding:5px; width:140px; background-color:#E7E7E7;">
	<div style="margin:5px; margin:5px; "><a href="posts.php?postID=<?=$resultLastHospitals[$i]['postID']?>" ><?=$resultLastHospitals[$i]['title']?></a></div>
  </div>
<?php
}
else 
