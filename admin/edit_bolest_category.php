...
if(isset($_REQUEST['bcID']) && !empty($_REQUEST['bcID']))			$bcID = $_REQUEST['bcID'];
...
exit;
   ...
   require_once("classes/BolestCategory.class.php");
   ...
   if(isset($add) && ($confirmOK == 1)) {
      ...
      if(!isset($Error)) {
         $BolestCtgr            = new BolestCategory($conn);
         $BolestCtgr->name      = $catName;
         $BolestCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
         if($BolestCtgr->create())
            $typeID = $BolestCtgr->id;
         else
            $Error .= errorMsg($BolestCtgr->Error);
      }
   }

   /*== UPDATE COMPANY CATEGORY ==*/
   if (isset($save) && ($bcID > 0) && ($confirmOK == 1)) {
      $catName = trim(htmlspecialchars($catName));
      ...
      if(!isset($Error)) {
         ...
         if (!$BolestCtgr->update())
            $Error .= errorMsg($BolestCtgr->Error);
      }
   }

   if (isset($bcID) && $bcID > 0) {
      $BolestCtgr = "Редактиране";
      $BolestCtgr     = new BolestCategory($conn);
      $BolestCtgr->id = $bcID;
      $BolestCtgr->load();
   } else {
...
<form name = "searchform" method = "POST" action = "edit_bolest_category.php" onsubmit = "return checkForCorrectData();">


<input type = "hidden" name = "ActionBolestCtgr" value="<?=$_REQUEST['ActionBolestCtgr']?>">
...
<div id="CONTAINER" style="margin:0px;width:auto; ">
  ...
  <div id="CENTER" style="margin-left:0px;">
   ...
   ?>
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         <input type = "radio" id = "kind1" name = "kind" value = "1"<?php print ((($BolestCtgr->parentID == 0) || ($bcID == 0)) ? " checked" : ""); ?>><label for = "kind1">главна категория</label>
         <?php
            $sql = "SELECT id, name FROM bolest_category WHERE parentID = 0 ORDER BY id";
            $conn->setsql($sql);
            $conn->getTableRows();
            if($conn->numberrows > 1 ) {
               printf("&nbsp;&nbsp;&nbsp;<input type = \"radio\" id = \"kind2\" name = \"kind\" value = \"2\"%s><label for = \"kind2\">подкатегория на</label>&nbsp;", (($BolestCtgr->parentID > 0) ? " checked" : ""));

               print "<select name = \"parentID\">\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $BolestCtgr->parentID) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
