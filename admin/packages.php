...
<legend> MySecondHome Пакети</legend>          
            <?php
               $sql = "SELECT pp.id as 'id', f.id as 'firm_id', f.name as 'firm', p.name as 'package' , IF(p.months=3,'3 Месечен','12 Месечен') as 'per_month',  p.total_price  as 'package_price', pp.is_payed as 'is_payed', pp.active as 'active', pp.start_date as 'start_date', pp.end_date as 'end_date' , IF((pp.start_date + INTERVAL 3 DAY)> NOW(),1,0) as 'tree_days',  IF((pp.end_date > NOW()),1,0) as 'expired' FROM purchased_packages pp, packages p, firms f WHERE pp.company_id=f.id AND pp.package_id=p.id $And_Company $And_Only_Not_Payed_Package ORDER BY firm";
               $conn->setsql($sql);
               $conn->getTableRows();
