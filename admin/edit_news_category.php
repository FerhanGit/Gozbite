...
if(isset($_REQUEST['ncID']) && !empty($_REQUEST['ncID']))			$ncID = $_REQUEST['ncID'];
...
exit;
   ...
   require_once("classes/NewsCategory.class.php");
   ...
   if(isset($add) && ($confirmOK == 1)) {
      ...
      if(!isset($Error)) {
         $NewsCtgr            = new NewsCategory($conn);
         $NewsCtgr->name      = $catName;
         $NewsCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
         if($NewsCtgr->create())
            $typeID = $NewsCtgr->id;
         else
            $Error .= errorMsg($NewsCtgr->Error);
      }
   }

   /*== UPDATE COMPANY CATEGORY ==*/
   if (isset($save) && ($ncID > 0) && ($confirmOK == 1)) {
      $catName = trim(htmlspecialchars($catName));
   ...
   if (isset($ncID) && $ncID > 0) {
      $NewsCtgr = "Редактиране";
      $NewsCtgr     = new NewsCategory($conn);
      $NewsCtgr->id = $ncID;
      $NewsCtgr->load();
   } else {
...
<form name = "searchform" method = "POST" action = "edit_news_category.php" onsubmit = "return checkForCorrectData();">


<input type = "hidden" name = "ActionNewsCtgrCtgr" value="<?=$_REQUEST['ActionNewsCtgrCtgr']?>">
...
<div id="CONTAINER" style="margin:0px;width:auto; ">
  ...
  <div id="CENTER" style="margin-left:0px;">
   ...
   ?>
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         <input type = "radio" id = "kind1" name = "kind" value = "1"<?php print ((($NewsCtgr->parentID == 0) || ($ncID == 0)) ? " checked" : ""); ?>><label for = "kind1">главна категория</label>
         <?php
            $sql = "SELECT id, name FROM news_category WHERE parentID = 0 ORDER BY id";
            $conn->setsql($sql);
            $conn->getTableRows();
            if($conn->numberrows > 1 ) {
               printf("&nbsp;&nbsp;&nbsp;<input type = \"radio\" id = \"kind2\" name = \"kind\" value = \"2\"%s><label for = \"kind2\">подкатегория на</label>&nbsp;", (($NewsCtgr->parentID > 0) ? " checked" : ""));

               print "<select name = \"parentID\">\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $NewsCtgr->parentID) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
