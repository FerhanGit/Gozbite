...
if(isset($_REQUEST['lcID']) && !empty($_REQUEST['lcID']))			$lcID = $_REQUEST['lcID'];
...
exit;
   ...
   require_once("classes/LekarstvoCategory.class.php");
   ...
   if(isset($add) && ($confirmOK == 1)) {
      ...
      if(!isset($Error)) {
         $LekarstvoCtgr            = new LekarstvoCategory($conn);
         $LekarstvoCtgr->name      = $catName;
         $LekarstvoCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
         if($LekarstvoCtgr->create())
            $typeID = $LekarstvoCtgr->id;
         else
            $Error .= errorMsg($LekarstvoCtgr->Error);
      }
   }

   /*== UPDATE COMPANY CATEGORY ==*/
   if (isset($save) && ($lcID > 0) && ($confirmOK == 1)) {
      $catName = trim(htmlspecialchars($catName));
      ...
      if(!isset($Error)) {
         $LekarstvoCtgr            = new LekarstvoCategory($conn);
         $LekarstvoCtgr->id        = $lcID;
         $LekarstvoCtgr->name      = $catName;
         $LekarstvoCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
      
         if (!$LekarstvoCtgr->update())
            $Error .= errorMsg($LekarstvoCtgr->Error);
      }
   }

   if (isset($lcID) && $lcID > 0) {
      $LekarstvoCtgr = "Редактиране";
      $LekarstvoCtgr     = new LekarstvoCategory($conn);
      $LekarstvoCtgr->id = $lcID;
      $LekarstvoCtgr->load();
   } else {
...
<form name = "searchform" method = "POST" action = "edit_lekarstvo_category.php" onsubmit = "return checkForCorrectData();">


<input type = "hidden" name = "ActionLekarstvoCtgr" value="<?=$_REQUEST['ActionLekarstvoCtgr']?>">
...
<div id="CONTAINER" style="margin:0px;width:auto; ">
  ...
  <div id="CENTER" style="margin-left:0px;">
   ...
   ?>
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         <input type = "radio" id = "kind1" name = "kind" value = "1"<?php print ((($LekarstvoCtgr->parentID == 0) || ($lcID == 0)) ? " checked" : ""); ?>><label for = "kind1">главна категория</label>
         <?php
            $sql = "SELECT id, name FROM lekarstva_category WHERE parentID = 0 ORDER BY id";
            $conn->setsql($sql);
            $conn->getTableRows();
            if($conn->numberrows > 1 ) {
               printf("&nbsp;&nbsp;&nbsp;<input type = \"radio\" id = \"kind2\" name = \"kind\" value = \"2\"%s><label for = \"kind2\">подкатегория на</label>&nbsp;", (($LekarstvoCtgr->parentID > 0) ? " checked" : ""));

               print "<select name = \"parentID\">\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $LekarstvoCtgr->parentID) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
