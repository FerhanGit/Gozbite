...
if(isset($_REQUEST['pcID']) && !empty($_REQUEST['pcID']))			$pcID = $_REQUEST['pcID'];
...
exit;
   ...
   require_once("classes/PostCategory.class.php");
   ...
   if(isset($add) && ($confirmOK == 1)) {
      ...
      if(!isset($Error)) {
         $PostCtgr            = new PostCategory($conn);
         $PostCtgr->name      = $catName;
         $PostCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
         if($PostCtgr->create())
            $typeID = $PostCtgr->id;
         else
            $Error .= errorMsg($PostCtgr->Error);
      }
   }

   /*== UPDATE COMPANY CATEGORY ==*/
   if (isset($save) && ($pcID > 0) && ($confirmOK == 1)) {
      $catName = trim(htmlspecialchars($catName));
 ...
 <script language = "JavaScript">
      function checkForCorrectData() {
         if(document.searchform.catName.value.length == 0) {
            alert('Моля, въведете наименование на фирмената категория!');
            document.searchform.catName.focus();
            return false;
         }

         if(document.searchform.catName.value.length > 64) {
            alert('Наименованието на фирмената категория не може да бъде по-дълго от 64 символа. Моля, коригирайте!');
            document.searchform.catName.focus();
            return false;
         }
...
<form name = "searchform" method = "POST" action = "edit_post_category.php" onsubmit = "return checkForCorrectData();">


<input type = "hidden" name = "ActionPostCtgr" value="<?=$_REQUEST['ActionPostCtgr']?>">
...
<div id="CONTAINER" style="margin:0px;width:auto; ">
  ...
  <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
   ...
   ?>
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         <input type = "radio" id = "kind1" name = "kind" value = "1"<?php print ((($PostCtgr->parentID == 0) || ($pcID == 0)) ? " checked" : ""); ?>><label for = "kind1">главна категория</label>
         <?php
            $sql = "SELECT id, name FROM post_category WHERE parentID = 0 ORDER BY id";
            $conn->setsql($sql);
            $conn->getTableRows();
            if($conn->numberrows > 1 ) {
               printf("&nbsp;&nbsp;&nbsp;<input type = \"radio\" id = \"kind2\" name = \"kind\" value = \"2\"%s><label for = \"kind2\">подкатегория на</label>&nbsp;", (($PostCtgr->parentID > 0) ? " checked" : ""));

               print "<select name = \"parentID\">\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $PostCtgr->parentID) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
