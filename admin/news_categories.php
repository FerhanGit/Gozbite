...
exit;
  ...
  if (isset($_REQUEST['ActionNewsCtgr']) && ($_REQUEST['ActionNewsCtgr'] == 1) && isset($_REQUEST['ncID']) && ($_REQUEST['ncID'] > 0)) {
      require("classes/NewsCategory.class.php");

      $NewsCtgr = new NewsCategory($conn);
      $NewsCtgr->id = $_REQUEST['ncID'];
      if (!$NewsCtgr->deleteNewsCtgr())
...
<input type = "hidden" name = "ActionNewsCtgr">
<input type = "hidden" name = "ncID">
     ...
     <div id="BANER_KVADRAT_AND_NEWS"style="float:left; width:650px; margin-left:10px; margin-top:10px;background-color:#F9FFF9;">
      ...
      <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;НОВИНИ | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newNewsType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_news_category.php?ncID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/NewsCategoriesList.class.php");
	            $CCList = new NewsCategoriesList($conn);
