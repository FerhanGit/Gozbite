...
exit;
  ...
  if (isset($_REQUEST['ActionBolestCtgr']) && ($_REQUEST['ActionBolestCtgr'] == 1) && isset($_REQUEST['bcID']) && ($_REQUEST['bcID'] > 0)) {
      require("classes/BolestCategory.class.php");

      $BolestCtgr = new BolestCategory($conn);
      $BolestCtgr->id = $_REQUEST['bcID'];
      if (!$BolestCtgr->deleteBolestCtgr())
...
<input type = "hidden" name = "ActionBolestCtgr">
  ...
  <div id="CENTER" style="margin-left:0px;">
     ...
     <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;БОЛЕСТИ | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newPostType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_bolest_category.php?bcID=0'" value="нова категория"/><br><br>
	         <?php
