...
exit;
  ...
  if (isset($_REQUEST['ActionOfferCtgr']) && ($_REQUEST['ActionOfferCtgr'] == 1) && isset($_REQUEST['ocID']) && ($_REQUEST['ocID'] > 0)) {
      require("classes/OfferCategory.class.php");

      $OfferCtgr = new OfferCategory($conn);
      $OfferCtgr->id = $_REQUEST['ocID'];
      if (!$OfferCtgr->deleteOfferCtgr())
...
<input type = "hidden" name = "ActionOfferCtgr">
<input type = "hidden" name = "ocID">
  ...
  <div id="CENTER" style="margin-left:0px;">
     ...
     <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;Почивки | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newPostType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_offer_category.php?ocID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/OfferCategoriesList.class.php");
	            $CCList = new OfferCategoriesList($conn);
