...
exit;
  ...
  if (isset($_REQUEST['ActionHotelCtgr']) && ($_REQUEST['ActionHotelCtgr'] == 1) && isset($_REQUEST['hcID']) && ($_REQUEST['hcID'] > 0)) {
      require("classes/HotelCategory.class.php");

      $HotelsCtgr = new HotelCategory($conn);
      $HotelsCtgr->id = $_REQUEST['hcID'];
      if (!$HotelsCtgr->deleteHotelCtgr())
         $Error = errorMsg($HotelsCtgr->Error);
   }
?> 
...
<input type = "hidden" name = "ActionHotelCtgr">
     ...
     <div id="BANER_KVADRAT_AND_NEWS"style="float:left; width:650px; margin-left:10px; margin-top:10px;background-color:#F9FFF9;">
      ...
      <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;ЛЕКАРИ | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newHotelType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_hotel_category.php?hcID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/HotelCategoriesList.class.php");
	            $CCList = new HotelCategoriesList($conn);
