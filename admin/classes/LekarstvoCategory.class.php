<?php
   class LekarstvoCategory {
      var $conn;
      var $id, $parentID, $name;
      var $Error;

      /*------------------ CONSTRUCTOR---------------------*/
      function LekarstvoCategory($conn) {
         $this->conn = $conn;
      } //CompanyCategory
      ...
      function prepareLoadQuery() {
         if(isset($this->id) && ($this->id > 0)) {
            $sql = sprintf("SELECT id, parentID, name FROM lekarstva_category WHERE id = %d", $this->id);
      ...
      function create() {
         ...
         $sql = sprintf("INSERT INTO lekarstva_category (parentID, name) VALUES (%d, '%s')", $this->parentID, $this->name);
         $this->conn->setsql($sql);
      ...
      function update() {
         ...
         $sql = sprintf("UPDATE lekarstva_category SET parentID = %d, name = '%s' WHERE id = %d", $this->parentID, $this->name, $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
      ...
      function deleteLekarstvoCtgr() {
         ...
         $sql = sprintf("SELECT lekarstvo_id FROM lekarstva_category_list WHERE category_id = %d", $this->id);
         $this->conn->setsql($sql);
         if($this->conn->getTableRow()) {
            $this->Error["SQL ERROR ClssCmpnCtgrDltCmpnCtgr"] = "Категорията не може да бъде изтрита! Има свързани с нея лекарства!";
            return false;
         }

         $sql = sprintf("DELETE FROM lekarstva_category WHERE id = %d", $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
