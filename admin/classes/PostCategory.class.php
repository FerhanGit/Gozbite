   ...
   class PostCategory {
      ...
      function prepareLoadQuery() {
         if(isset($this->id) && ($this->id > 0)) {
            $sql = sprintf("SELECT id, parentID, name FROM post_category WHERE id = %d", $this->id);
      ...
      function create() {
         ...
         $sql = sprintf("INSERT INTO post_category (parentID, name) VALUES (%d, '%s')", $this->parentID, $this->name);
         $this->conn->setsql($sql);
      ...
      function update() {
         ...
         $sql = sprintf("UPDATE post_category SET parentID = %d, name = '%s' WHERE id = %d", $this->parentID, $this->name, $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
      ...
      function deletePostCtgr() {
         ...
         $sql = sprintf("SELECT postID FROM posts WHERE post_category = %d", $this->id);
         $this->conn->setsql($sql);
         ...
         $sql = sprintf("DELETE FROM post_category WHERE id = %d", $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
