<?php
   ...
   $response = "";
        ...
        $sql="SELECT * FROM hospital_comment WHERE firmID='".$_REQUEST['all_firmID']."' ORDER BY created_on DESC";
		$conn->setsql($sql);
		$conn->getTableRows();
      ...
      if($numComment > 0) {
          for($k=0;$k<$numComment;$k++)
          {
                    ...
                    $response .= "  <input type='hidden' name='firmID[]' value=".$resultComment[$k]['firmID'].">";
