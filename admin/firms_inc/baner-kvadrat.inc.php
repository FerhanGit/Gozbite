<div id="BANER_KVADRAT_AND_NEWS_DIV" style="float:left;padding:0px 0px 0px 0px;">
   	...
   	<div id="NEWS" style="float:left; width:330px;margin-left:10px;color:#FF8400;">
			 <?php
				   ...
				   $sql="SELECT h.id as 'id' FROM hospitals h ORDER BY h.registered_on DESC LIMIT 5";
				   $conn->setsql($sql);
				   $conn->getTableRows();  
			...
			<?php
					if($itmNum > 0)
					{
						for ($i=0;$i<$itmNum;$i++)
					   	{		   			
			?>
							<a href="hospitals.php?firmID=<?=$Itm2[$i]?>" target="_blank" style="text-decoration:none;" ><div id="newsButton<?=$i?>" class="newsButton" style="float:left;margin:2px 10px 2px 0px;width:30px;height:20px;color:#ffffff;background-color:#009900;cursor:pointer;cursor:hand;" align="center" onmouseover="window.clearInterval(t);onMouseOverUpdate(<?=$i?>);" onmouseout="t=window.setInterval('autoUpdate()', 10000);count=<?=(($i+1)<5)?($i+1):0?>;" ><?=($i+1)?></div></a>
			<?php
					   	}
					}
			...
			<script type="text/javascript">
				...
				function autoUpdate()
				{
					new Ajax.Updater('newsDIV', 'hospitals_inc/Ajax_hospitals_ticker.php?Itms=<?=implode(",",$Itm2)?>&index='+count, {
				...
				{
					new Ajax.Updater('newsDIV', 'hospitals_inc/Ajax_hospitals_ticker.php?Itms=<?=implode(",",$Itm2)?>&index='+count2, {
					  method: 'get',onSuccess: function() {
					  	 			   new Effect.Opacity('newsDIV', {duration: 1.0, from:0.3, to:1.0});
				                       new Effect.Opacity('newsDIVContainer', {duration: 1.0, from:0.3, to:1.0});
				              			}
