<?php
   ...
   $response = "";
        ...
        $sql="SELECT * FROM bolest_comment WHERE bolestID='".$_REQUEST['all_bolestID']."' ORDER BY created_on DESC";
		$conn->setsql($sql);
		$conn->getTableRows();
      ...
      if($numComment > 0) {
          for($k=0;$k<$numComment;$k++)
          {
                    ...
                    $response .= "  <input type='hidden' name='bolestID[]' value=".$resultComment[$k]['bolestID'].">";
