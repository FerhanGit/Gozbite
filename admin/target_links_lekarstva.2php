...
if ((isset($_POST['vavedi'])) && (!empty($_POST['fromDate'])) && (!empty($_POST['toDate'])))
{ 
	
	
	$sql="INSERT INTO target_links_lekarstva SET doctor_id='".$_POST['doctorID']."',
											 firm_id='".$_POST['companyID']."',
											 category='".$_POST['lekarstvo_category']."',
    ...
    $last_ID=$conn->insertDB();
     ...
     if (isset($_REQUEST['lekarstvo_simptom']))
	{
		if (!empty($_POST['lekarstvo_simptom']) && is_array($_POST['lekarstvo_simptom'])) $simptom=implode(',',$_POST['lekarstvo_simptom']);
		$sql="UPDATE target_links_lekarstva SET simptom='".$simptom."' WHERE id='".$last_ID."'";
		$conn->setsql($sql);
		$conn->updateDB();
...
{		
	...
	$sql="DELETE FROM target_links_lekarstva WHERE id='".$_REQUEST['delete']."'"; 
	$conn->setsql($sql);
    $conn->updateDB();
