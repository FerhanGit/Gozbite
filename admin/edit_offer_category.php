...
if(isset($_REQUEST['ocID']) && !empty($_REQUEST['ocID']))			$ocID = $_REQUEST['ocID'];
...
exit;
   ...
   require_once("classes/OfferCategory.class.php");
   ...
   if(isset($add) && ($confirmOK == 1)) {
      ...
      if(!isset($Error)) {
         $OfferCtgr            = new OfferCategory($conn);
         $OfferCtgr->name      = $catName;
         $OfferCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
         if($OfferCtgr->create())
            $typeID = $OfferCtgr->id;
         else
            $Error .= errorMsg($OfferCtgr->Error);
      }
   }

   /*== UPDATE COMPANY CATEGORY ==*/
   if (isset($save) && ($ocID > 0) && ($confirmOK == 1)) {
      $catName = trim(htmlspecialchars($catName));
   ...
   if (isset($ocID) && $ocID > 0) {
      $OfferCtgr = "Редактиране";
      $OfferCtgr     = new OfferCategory($conn);
      $OfferCtgr->id = $ocID;
      $OfferCtgr->load();
   } else {
...
<form name = "searchform" method = "POST" action = "edit_offer_category.php" onsubmit = "return checkForCorrectData();">


<input type = "hidden" name = "ActionOfferCtgr" value="<?=$_REQUEST['ActionOfferCtgr']?>">
...
<div id="CONTAINER" style="margin:0px;width:auto; ">
  ...
  <div id="LEFT-1" style="float:left; width:160px;margin:0px;">
	  <?php include("offers_inc/left-1.php");  ?>  
  </div>


  
  <div id="LEFT-2" style="float:left; width:150px;margin:0px;"> 
  	  <?php include("offers_inc/left-2.php");  ?>  
  </div>
  ...
  <div id="CENTER" style="margin-left:0px;">
     ...
     <div id="BANER_KVADRAT_AND_NEWS"style="float:left; width:650px; margin-left:10px; margin-top:10px;background-color:#F9FFF9;">
       <?php  include("offers_inc/baner-kvadrat.inc.php");  ?>  
     </div>
   ...
   ?>
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         <input type = "radio" id = "kind1" name = "kind" value = "1"<?php print ((($OfferCtgr->parentID == 0) || ($ocID == 0)) ? " checked" : ""); ?>><label for = "kind1">главна категория</label>
         <?php
            $sql = "SELECT id, name FROM offer_category WHERE parentID = 0 ORDER BY id";
            $conn->setsql($sql);
            $conn->getTableRows();
            if($conn->numberrows > 1 ) {
               printf("&nbsp;&nbsp;&nbsp;<input type = \"radio\" id = \"kind2\" name = \"kind\" value = \"2\"%s><label for = \"kind2\">подкатегория на</label>&nbsp;", (($OfferCtgr->parentID > 0) ? " checked" : ""));

               print "<select name = \"parentID\">\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $OfferCtgr->parentID) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
     ...
     <div id="RIGHT" style="float:left;margin-left:10px; width:150px;margin-top:20px;">
        <?php include("offers_inc/right.php");  ?>  
     </div>      
  </div>
