...
{ 
	...
	$sql="INSERT INTO target_links_bolesti SET doctor_id='".$_POST['doctorID']."',
											 firm_id='".$_POST['companyID']."',
											 category='".$_POST['bolest_category']."',
	...
	{
		if (!empty($_POST['bolest_simptom']) && is_array($_POST['bolest_simptom'])) $simptom=implode(',',$_POST['bolest_simptom']);
		$sql="UPDATE target_links_bolesti SET simptom='".$simptom."' WHERE id='".$last_ID."'";
		$conn->setsql($sql);
		$conn->updateDB();
...
{		
	...
	$sql="DELETE FROM target_links_bolesti WHERE id='".$_REQUEST['delete']."'"; 
	$conn->setsql($sql);
    $conn->updateDB();
