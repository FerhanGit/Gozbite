...
if(isset($_REQUEST['sendMails']))
{
  ...
  // -------------------------------------- FIRMS ------------------------------------------------
	...
	{
		$sql="SELECT f.id as 'firmID', f.name as 'firm_name', f.email as 'email' FROM firms f, firm_category fc, firms_category_list fcl WHERE fcl.firm_id = f.id AND fcl.category_id = fc.id AND (fcl.category_id = '".$_REQUEST['firmCat']."' OR fcl.category_id IN (SELECT id FROM firm_category WHERE parentID='".$_REQUEST['firmCat']."')) AND f.name <> '' AND LENGTH(f.email) > 7 ";
		$conn->setsql($sql);
		$conn->getTableRows();
	...
	{
		$sql = "SELECT f.id as 'firmID', f.name as 'firm_name', f.email as 'email' FROM firms f WHERE f.name <> '' AND LENGTH(f.email) > 7 ";
		$conn->setsql($sql);
		$conn->getTableRows();
		$resMailsFirms = $conn->result;
		$numMailsFirms = $conn->numberrows;
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         ...
         <?php
            ...
            if($conn->numberrows > 1 ) {
               print "<select name = \"firmCat\">\n";
               print "<option value = \"\" >нито една категория</option>\n";
               print "<option value = \"-1\" >всички категории фирми</option>\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $_REQUEST['firmCat']) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
