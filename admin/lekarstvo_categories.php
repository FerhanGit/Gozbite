...
exit;
  ...
  if (isset($_REQUEST['ActionLekarstvoCtgr']) && ($_REQUEST['ActionLekarstvoCtgr'] == 1) && isset($_REQUEST['lcID']) && ($_REQUEST['lcID'] > 0)) {
      require("classes/LekarstvoCategory.class.php");

      $LekarstvoCtgr = new LekarstvoCategory($conn);
      $LekarstvoCtgr->id = $_REQUEST['lcID'];
      if (!$LekarstvoCtgr->deleteLekarstvoCtgr())
...
<input type = "hidden" name = "ActionLekarstvoCtgr">
  ...
  <div id="CENTER" style="margin-left:0px;">
     ...
     <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;ЛЕКАРСТВА | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newPostType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_lekarstvo_category.php?lcID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/LekarstvoCategoriesList.class.php");
	            $CCList = new LekarstvoCategoriesList($conn);
