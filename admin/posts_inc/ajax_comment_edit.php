<?php
   ...
   $response = "";
        ...
        $sql="SELECT * FROM post_comment WHERE post_id='".$_REQUEST['all_postID']."' ORDER BY created_on DESC";
		$conn->setsql($sql);
		$conn->getTableRows();
      ...
      if($numComment > 0) {
          for($k=0;$k<$numComment;$k++)
          {
                    ...
                    $response .= "  <input type='hidden' name='postID[]' value=".$resultComment[$k]['post_id'].">";
