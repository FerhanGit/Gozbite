...
exit;
  ...
  if (isset($_REQUEST['ActionFirmCtgr']) && ($_REQUEST['ActionFirmCtgr'] == 1) && isset($_REQUEST['fcID']) && ($_REQUEST['fcID'] > 0)) {
      require("classes/FirmCategory.class.php");

      $FirmCtgr = new FirmCategory($conn);
      $FirmCtgr->id = $_REQUEST['fcID'];
      if (!$FirmCtgr->deleteFirmCtgr())
...
<input type = "hidden" name = "ActionFirmCtgr">
<input type = "hidden" name = "fcID">
     ...
     <div id="BANER_KVADRAT_AND_NEWS"style="float:left; width:650px; margin-left:10px; margin-top:10px;background-color:#F9FFF9;">
      ...
      <div id="MAIN" style="float:left; width:480px; margin:20px; margin-right:10px; background-color:#F5F5F5;" align="center">
	   ...
	   <fieldset style="margin:10px;">
	      <legend>&nbsp;Административен модул&nbsp;&nbsp;ЗДРАВНИ ЗАВЕДЕНИЯ | К А ТЕ Г О Р И И&nbsp;</legend>
	      <div style = "margin: 10px 10px 10px 10px;">
	         <input type="button" name = "newFirmType" title = "Модул за добавяне на категория" class = "buttonInv" onclick = "document.location.href = 'edit_firm_category.php?fcID=0'" value="нова категория"/><br><br>
	         <?php
	            require_once("classes/FirmCategoriesList.class.php");
