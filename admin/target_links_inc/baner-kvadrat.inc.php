<div id="BANER_KVADRAT_AND_NEWS_DIV" style="float:left;padding:0px 0px 0px 0px;">
   	...
   	<div id="NEWS" style="float:left; width:330px;margin-left:10px;color:#FF8400;">
			...
			<?php
					if($itmNum > 0)
					{
						for ($i=0;$i<$itmNum;$i++)
					   	{		   			
			?>
							<a href="news.php?newsID=<?=$Itm2[$i]?>" target="_blank" style="text-decoration:none;" ><div id="newsButton<?=$i?>" class="newsButton" style="float:left;margin:2px 10px 2px 0px;width:30px;height:20px;color:#ffffff;background-color:#467B99;cursor:pointer;cursor:hand;" align="center" onmouseover="window.clearInterval(t);onMouseOverUpdate(<?=$i?>);" onmouseout="t=window.setInterval('autoUpdate()', 10000);count=<?=(($i+1)<5)?($i+1):0?>;" ><?=($i+1)?></div></a>
			<?php
					   	}
					}
			...
			?>
				...
				<div style="float:left;">
					<div class="newsDIVContainer" id="newsDIVContainer" style="width:320px;float:left;background-color:lightblue;margin-top:10px;margin-bottom:0px;">
						<div id="newsDIV" style="width:320px;height:250px;background-color:lightblue;overflow:hidden;"></div>
					</div>
			...
			<script type="text/javascript">
				...
				function autoUpdate()
				{
					new Ajax.Updater('newsDIV', 'news_inc/Ajax_news_ticker.php?Itms=<?=implode(",",$Itm2)?>&index='+count, {
					...
					$('newsButton0').setStyle({backgroundColor: '#467B99'});
					$('newsButton1').setStyle({backgroundColor: '#467B99'});
					$('newsButton2').setStyle({backgroundColor: '#467B99'});
					$('newsButton3').setStyle({backgroundColor: '#467B99'});
					$('newsButton4').setStyle({backgroundColor: '#467B99'});
				...
				{
					new Ajax.Updater('newsDIV', 'news_inc/Ajax_news_ticker.php?Itms=<?=implode(",",$Itm2)?>&index='+count2, {
					  method: 'get',onSuccess: function() {
					  	 			   new Effect.Opacity('newsDIV', {duration: 1.0, from:0.3, to:1.0});
				                       new Effect.Opacity('newsDIVContainer', {duration: 1.0, from:0.3, to:1.0});
				              			}
   	...
   	<div id="BANER_KVADRAT" style="float:left; width:310px;padding-right:0px;overflow:hidden;">
   		...
   		<div style="float:right;background-image:url(images/reklama_<?=$theme_color?>.png);margin-bottom:5px; height:28px; width:144px; background-repeat:no-repeat; font-size:12px; color:#FFFFFF;"></div>
