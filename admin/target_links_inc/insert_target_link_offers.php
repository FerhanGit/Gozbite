       ...
       <div style = "margin: 10px 10px 10px 10px;">
          ...
          <div style = "margin: 10px 10px 10px 10px;">
            ...
            ?>
 			...
 			<select style="width:200px;" name="offer_category" id="offer_category" >				
            <option value ="">избери</option>		                            
             <?php
                             
               $sql = sprintf("SELECT id, name FROM offer_category ORDER BY name");
               $conn->setSQL($sql);
               $conn->getTableRows();
...
if ((!isset($_POST['loc_id']))&&(!isset($_POST['start_date']))&&(!isset($_POST['end_date'])))
{   
		
	$sql="SELECT DISTINCT(tl.id) as 'id', tl.category as 'category', tl.firm_id as 'firm', tl.hotel_id as 'hotel', tl.loc_id as 'location', l.name as 'location_name',  lt.name as 'locType', tl.transport_type as 'transport_type', tl.fromD as 'fromD', tl.toD as 'toD'  FROM target_links_offers tl, transports tr, offer_category oc, locations l, location_types lt WHERE (lt.id=l.loc_type_id AND tl.loc_id = l.id  AND tl.loc_id <> 0) OR oc.id=tl.category OR tr.id = tl.transport_type ORDER BY toD"; 
	$conn->setsql($sql);
    $conn->getTableRows();
    $result=$conn->result;
    $num=$conn->numberrows;
    ...
    // -------------------------------- Zaqvkite -----------------------------------------
 		...
 		$sql="SELECT id,name FROM offer_category WHERE id IN (".$result[$i]['category'].")"; 
		$conn->setsql($sql);
   		$conn->getTableRows();
