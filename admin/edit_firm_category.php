...
if(isset($_REQUEST['fcID']) && !empty($_REQUEST['fcID']))			$fcID = $_REQUEST['fcID'];
...
exit;
   ...
   require_once("classes/FirmCategory.class.php");
   ...
   if(isset($add) && ($confirmOK == 1)) {
      ...
      if(!isset($Error)) {
         $FirmCtgr            = new FirmCategory($conn);
         $FirmCtgr->name      = $catName;
         $FirmCtgr->parentID  = (($kind == 1) ? 0 : $parentID);
         if($FirmCtgr->create())
            $typeID = $FirmCtgr->id;
         else
            $Error .= errorMsg($FirmCtgr->Error);
      }
   }

   /*== UPDATE COMPANY CATEGORY ==*/
   if (isset($save) && ($fcID > 0) && ($confirmOK == 1)) {
      $catName = trim(htmlspecialchars($catName));
   ...
   if (isset($fcID) && $fcID > 0) {
      $FirmCtgr = "Редактиране";
      $FirmCtgr     = new FirmCategory($conn);
      $FirmCtgr->id = $fcID;
      $FirmCtgr->load();
   } else {
...
<form name = "searchform" method = "POST" action = "edit_firm_category.php" onsubmit = "return checkForCorrectData();">


<input type = "hidden" name = "ActionFirmCtgrCtgr" value="<?=$_REQUEST['ActionFirmCtgrCtgr']?>">
...
<div id="CONTAINER" style="margin:0px;width:auto; ">
  ...
  <div id="LEFT-1" style="float:left; width:160px;margin:0px;">
	  <?php include("firms_inc/left-1.php");  ?>  
  </div>


  
  <div id="LEFT-2" style="float:left; width:150px;margin:0px;"> 
  	  <?php include("firms_inc/left-2.php");  ?>  
  </div>
  ...
  <div id="CENTER" style="margin-left:0px;">
     ...
     <div id="BANER_KVADRAT_AND_NEWS"style="float:left; width:650px; margin-left:10px; margin-top:10px;background-color:#F9FFF9;">
       <?php  include("firms_inc/baner-kvadrat.inc.php");  ?>  
     </div>
   ...
   ?>
      ...
      <div style = "margin: 10px 10px 10px 10px;">
         <input type = "radio" id = "kind1" name = "kind" value = "1"<?php print ((($FirmCtgr->parentID == 0) || ($fcID == 0)) ? " checked" : ""); ?>><label for = "kind1">главна категория</label>
         <?php
            $sql = "SELECT id, name FROM firm_category WHERE parentID = 0 ORDER BY id";
            $conn->setsql($sql);
            $conn->getTableRows();
            if($conn->numberrows > 1 ) {
               printf("&nbsp;&nbsp;&nbsp;<input type = \"radio\" id = \"kind2\" name = \"kind\" value = \"2\"%s><label for = \"kind2\">подкатегория на</label>&nbsp;", (($FirmCtgr->parentID > 0) ? " checked" : ""));

               print "<select name = \"parentID\">\n";
               for($i = 0; $i < $conn->numberrows; $i++) {
                  printf(     "<option value = \"%d\"%s>%s</option>\n", $conn->result[$i]["id"], (($conn->result[$i]["id"] == $FirmCtgr->parentID) ? " selected" : ""), $conn->result[$i]["name"]);
               }
               print "</select>\n";
            }
         ?>
     ...
     <div id="RIGHT" style="float:left;margin-left:10px; width:150px;margin-top:20px;">
        <?php include("firms_inc/right.php");  ?>  
     </div>      
  </div>
