   ...
   class DrinkCategory {
      ...
      function prepareLoadQuery() {
         if(isset($this->id) && ($this->id > 0)) {
            $sql = sprintf("SELECT id, parentID, name FROM drink_category WHERE id = %d", $this->id);
      ...
      function create() {
         ...
         $sql = sprintf("INSERT INTO drink_category (parentID, name) VALUES (%d, '%s')", $this->parentID, $this->name);
         $this->conn->setsql($sql);
      ...
      function update() {
         ...
         $sql = sprintf("UPDATE drink_category SET parentID = %d, name = '%s' WHERE id = %d", $this->parentID, $this->name, $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
      ...
      function deleteDrinkCtgr() {
         ...
         $sql = sprintf("SELECT drink_id FROM drinks_category_list WHERE category_id = %d", $this->id);
         $this->conn->setsql($sql);
         ...
         $sql = sprintf("DELETE FROM drink_category WHERE id = %d", $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
