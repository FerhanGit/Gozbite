   ...
   class  FirmLocationsList {
      ...
      function showSelectMultipleList($parent, $indenter, $arr) {
         if(is_array($this->nodes[$parent])) {
            for(reset($this->nodes[$parent]); $id = key($this->nodes[$parent]); next($this->nodes[$parent])){
               if($this->nodes[$parent][$id]["id"] == 14880) continue;
               $return .= sprintf(" <option value = \"%d\"%s %s>%s&nbsp;%s %s</option>", $this->nodes[$parent][$id]["id"], ((in_array($this->nodes[$parent][$id]["id"], $arr)) ? " selected" : ""), (in_array($this->nodes[$parent][$id]["loc_type_id"], array(1, 18, 10, 11)) ? " DISABLED" : ""), $indenter, (($this->nodes[$parent][$id]["loc_type_id"] == 1) ? "област " : (($this->nodes[$parent][$id]["loc_type_id"] == 10) ? "община " : (($this->nodes[$parent][$id]["loc_type_id"] == 11) ? "кметство " : ""))), $this->nodes[$parent][$id]["name"]);
