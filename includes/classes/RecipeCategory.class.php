   ...
   class RecipeCategory {
      ...
      function prepareLoadQuery() {
         if(isset($this->id) && ($this->id > 0)) {
            $sql = sprintf("SELECT id, parentID, name FROM recipe_category WHERE id = %d", $this->id);
      ...
      function create() {
         ...
         $sql = sprintf("INSERT INTO recipe_category (parentID, name) VALUES (%d, '%s')", $this->parentID, $this->name);
         $this->conn->setsql($sql);
      ...
      function update() {
         ...
         $sql = sprintf("UPDATE recipe_category SET parentID = %d, name = '%s' WHERE id = %d", $this->parentID, $this->name, $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
      ...
      function deleteRecipeCtgr() {
         ...
         $sql = sprintf("SELECT recipe_id FROM recipes_category_list WHERE category_id = %d", $this->id);
         $this->conn->setsql($sql);
         if($this->conn->getTableRow()) {
            $this->Error["SQL ERROR ClssCmpnCtgrDltCmpnCtgr"] = "Категорията не може да бъде изтрита! Има свързани с нея рецепти!";
            return false;
         }

         $sql = sprintf("DELETE FROM recipe_category WHERE id = %d", $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
