   ...
   class FirmCategory {
      ...
      function prepareLoadQuery() {
         if(isset($this->id) && ($this->id > 0)) {
            $sql = sprintf("SELECT id, parentID, name FROM firm_category WHERE id = %d", $this->id);
      ...
      function create() {
         ...
         $sql = sprintf("INSERT INTO firm_category (parentID, name) VALUES (%d, '%s')", $this->parentID, $this->name);
         $this->conn->setsql($sql);
      ...
      function update() {
         ...
         $sql = sprintf("UPDATE firm_category SET parentID = %d, name = '%s' WHERE id = %d", $this->parentID, $this->name, $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
      ...
      function deleteFirmCtgr() {
         ...
         $sql = sprintf("SELECT firm_id FROM firms_category_list WHERE category_id = %d", $this->id);
         $this->conn->setsql($sql);
         ...
         $sql = sprintf("DELETE FROM firm_category WHERE id = %d", $this->id);
         $this->conn->setsql($sql);
         $this->conn->UpdateDB();
         if($this->conn->error) {
            for(reset($this->conn->error); $key = key($this->conn->error); next($this->conn->error)) {
