			 ...
			 $sql = sprintf("SELECT id as 'id' , title as 'title'
		      ...
		      if($numHints > 0) {
		         for($i = 0; $i < $numHints; $i++) {
		         	...
		         	?>
		             <li onClick = "updateDrinkSelect('<?=$resultHints[$i]['title']?>',<?=$resultHints[$i]['id']?>);fill('<?=$resultHints[$i]['title']?>');"><?=str_replace(ucfirst($queryString),'<font color="#FF6600;">'.ucfirst($queryString).'</font>',htmlspecialchars($resultHints[$i]['title']))?></li>
					<?php
		         }
		      }
		      else 
