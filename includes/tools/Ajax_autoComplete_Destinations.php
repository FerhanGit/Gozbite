			 ...
			 $sql = sprintf("SELECT id as 'id' , name as 'name'
		      ...
		      if($numHints > 0) {
		         for($i = 0; $i < $numHints; $i++) {
		         	...
		         	?>
		             <li onClick = "updateDestinationSelect('<?=$resultHints[$i]['name']?>',<?=$resultHints[$i]['id']?>);fill('<?=$resultHints[$i]['name']?>');"><?=str_replace(ucfirst($queryString),'<font color="#FF6600;">'.ucfirst($queryString).'</font>',htmlspecialchars(locationTracker($resultHints[$i]['id'])))?></li>
					<?php
		         }
		      }
		      else 
