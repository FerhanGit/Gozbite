 ...
 if (isset($_REQUEST['insert_comment_btn']))
 {
	...
	{		
		...
		$_SESSION['verification_key']="";
	     ...
	     if (!empty($_REQUEST['comment_body']) && $_REQUEST['drinkID'] > 0)
	     {
	       ...
	       $sql="INSERT INTO drink_comment SET drinkID='".$_REQUEST['drinkID']."',
	    	...
	    	if($last_ID>0)
		    {
		    	...
		    	$_SESSION['cnt_comment'] ++;
		    	  ...
		    	  // ==================================== MAIL SEND ============================================
				  		...
				  		$sql="SELECT drinkID, sender_name, sender_email FROM drink_comment WHERE drinkID = '".$_REQUEST['drinkID']."' GROUP BY sender_email";
						$conn->setsql($sql);
						$conn->getTableRows();
						...
						if($numCommentNew > 0)
						{
							for($n=0; $n<$numCommentNew; $n++)
							{
								...
								$body .= "Уважаеми, <b><font color='#FF6600'>".$resultCommentNew[$n]['sender_name']."</font></b>, Има нов коментар свързан с Вашето мнение относно напитка '".get_drink_nameBydrinkID($_REQUEST['drinkID'])."', публикувана в кулинарния портал GoZbiTe.Com!<br /><br />";
						   		
						   		 $body .= "<div style=\"width:600px; float:left; margin: 0px 0px 50px 0px;\">	
						   		 			За да го прочетете последвайте този линк <a href='http://GoZbiTe.Com/разгледай-напитка-".$_REQUEST['drinkID'].",".myTruncateToCyrilic(get_drink_nameBydrinkID($_REQUEST['drinkID']),50,'_','').".html'>Виж коментара</a>.
						   					</div>";
	    ...
	    {
	        $comment .= '<script type="text/javascript">alert("Не сте попълнили задължителните полета!");window.location.href=\'разгледай-напитка-'.$_REQUEST['drinkID'].','.myTruncateToCyrilic(get_drink_nameByDrinkID($_REQUEST['drinkID']),200,'_','').'.html\';</script> ';
   ...
   {
	    $comment .= '<script type="text/javascript">alert("Не сте попълнили Кода за сигурност!");window.location.href=\'разгледай-напитка-'.$_REQUEST['drinkID'].','.myTruncateToCyrilic(get_drink_nameByDrinkID($_REQUEST['drinkID']),200,'_','').'.html\';</script> ';
