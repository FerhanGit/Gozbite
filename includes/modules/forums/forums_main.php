...
$posts_main .=	'<script type="text/javascript">
						makeViewLog(\'post\',\''.$last_item['postID'].'\');
					</script>';
...
<p>
	...
	<div id="tabs_inside" style="width:660px;">
		...
		<h3 style="margin: 10px 0px 0px 0px; padding-left:5px; color: #FF6600; background: #F1F1F1 url(images/gradient_tile.png) repeat 0 -5px;"><a style=" font-size:14px; font-weight:bold;" href="прочети-статия-'.$last_item['postID'].','.myTruncateToCyrilic($last_item['title'],200,'_','') .'.html" >'.$last_item['title'].'</a></h3>
		<br />';
		...
		$posts_main .= '<a href="прочети-статия-'.$last_item['postID'].','.myTruncateToCyrilic($last_item['title'],200,'_','') .'.html" ><div  onMouseover="this.style.borderColor=\'#0099FF\';" onMouseout="this.style.borderColor=\'#CCCCCC\';"  style="float:left; margin:5px;  border:1px solid #CCCCCC; width:190px; padding:2px; background-color:#F9F9F9;" align="center" ><img width="'.($newwidth?$newwidth:180).'"  src="'.$picFile.'" /></div></a>
		<div style="float:right; width:250px; padding:5px; border-left:1px double #CDC8B4;">';
		$posts_main .= $response;
		$posts_main .= "</div>".strip_tags(myTruncate($last_item['body'], 2000, ' ', ' ... '),'<br><br /><a><p><b><strong>')."<a href='прочети-статия-".$last_item['postID'].",".myTruncateToCyrilic($last_item['title'],200,'_','') .".html'> <img src='images/promo_view_btn.gif' /></a>
