...
*/
	...
	log_post($postBig['postID']); // сложено е тук, защото за posts_main.php няма postID в main.php
//$post_big .=print_r($postBig,1);
...
<div class="detailsDiv" style="float:left; width:650px;margin-bottom:20px; border-top:3px solid #6DCA31; padding:5px; background-color:#D7F3CE;">
	<div style=" float:left; margin-left:6px; width:450px;color:#ff6600;font-weight:bold;" >'.$postBig['title'].'</div>
...
$post_big .= '<div style=" float:right; margin-right:5px; font-weight:bold; color:#ff6600;" ><u>Разгледано '.($postBig['cnt']?$postBig['cnt']:1).' пъти</u></div>';								
	...
	$post_big .= '	<div style="float:right; margin-right:5px;" ><a href="редактирай-статия-'.$postBig['postID'].','.myTruncateToCyrilic($postBig['title'],50,'_','') .'.html"><img style="margin-left:5px;" src="images/page_white_edit.png" width="14" height="14"></a></div>';
	}
	if((($_SESSION['user_kind'] == 2) or $_SESSION['userID']==1)) {	
	$post_big .= '	<div style="float:right; margin-right:5px;" ><a onclick="if(!confirm(\'Сигурни ли сте?\')){return false;}" href="изтрий-статия-'.$postBig['postID'].','.myTruncateToCyrilic($postBig['title'],50,'_','') .'.html"><img style="margin-left:5px;" src="images/page_white_delete.png" width="14" height="14"></a></div>';
	}
...
$post_big .= '<div class="detailsDiv" style="float:left; width:650px;margin-bottom:20px; border-top:3px solid #6DCA31; padding:5px; background-color:#D7F3CE;">
	<div style=" float:left; margin-right:5px; font-weight:bold; color:#6B9D09;" ><h3 align="left"><font style="font-size:12px;">Категория:</font> <a style=" font-size:14px; color:#6B9D09;" href="статии-категория-'.$postBig['category_id'].','.$page.','.myTruncateToCyrilic($postBig['category'],50,'_','') .'.html">'.$postBig['category'].'</a> | </h3></div>								
	<div style=" float:left; font-size:14px; margin-left:6px; width:250px;color:#6B9D09; " ><i>'.convert_Month_to_Cyr(date("j F Y,H:i:s",strtotime($postBig['date']))) .'</i></div>
	<div style=" float:right; font-size:14px; margin-right:5px; color:#6B9D09;" >Източник: '.$postBig['source'] .'</div>								
	<br style="clear:both;"/>							
</div>
...
<br style="clear:both;"/>	';
 ...
 if($postBig['numTags'] > 0) 
 {
 	$post_big .= '<table><tr><td> 
		     <div class="postBig">
				<div class="detailsDiv" style="float:left; width:640px;margin-bottom:20px; border-top:3px solid #6DCA31; padding:5px; background-color:#F1F1F1;">
...
<div id="starDiv" style=" float:left;width:200; margin-top:0px;"> </div>';

$post_big .= '<script language="javascript" type="text/javascript">
   new Starbox("starDiv", '.$postBig['rating'].', { rerate: false, max: 6, stars: 6, buttons: 12, color:"#FF6600",hoverColor:"#B9F4A8", total: '.$postBig['times_rated'].', indicator: " Рейтинг #{average} от #{total} гласа", ghosting: true ,onRate: function(element, info) {
...
$post_big .= 'function saveStar(event) {
			
	  new Ajax.Request("includes/tools/savestar.php?postID='.$_REQUEST['postID'].'", {
...
$post_big .= '<div style="float:right;margin-right:10px;">
            <div style="float:left; margin-right:2px;" >
				<fb:like href="http://www.ohboli.bg/прочети-статия-'.$postBig['postID'].','.myTruncateToCyrilic(strip_tags($postBig['title']),100,'_','') .'.html" layout="button_count"	show_faces="false" width="50" height="21" action="like" colorscheme="light"></fb:like>								
				<span style="background:transparent; color:#6B9D09; font-size:12px; font-weight:bold;">Сподели в: </span>
				<a href="http://www.facebook.com/sharer.php?u=http://www.ohboli.bg/прочети-статия-'.$postBig['postID'].','.myTruncateToCyrilic($postBig['title'],50,'_','') .'.html&amp;t='.$postBig['title'].'" target="_blank"><img src="images/facebook.png" border="0" alt="" title="" width="16" hspace="1" height="16" class="absmiddle" /></a>&nbsp;
				<a href="http://twitter.com/home?status='.myTruncateToCyrilic($postBig['title'],200,'_','') .', http://www.ohboli.bg/прочети-статия-'.$postBig['postID'].','.myTruncateToCyrilic($postBig['title'],50,'_','') .'.html" target="_blank"><img src="images/twitter.png" border="0" alt="" title="" width="16" hspace="1" height="16" class="absmiddle" /></a>&nbsp;<a href="http://www.google.com/buzz/post" data-locale="bg" target="_blank"><img src="i/ico/google.png" border="0" alt="" title="" width="16" hspace="1" height="16" class="absmiddle" /></a>
			</div>';
...
{ 
	$post_big .= '<div class="detailsDiv" style="float:left; width:640px; margin-bottom:20px; margin-right:5px; border-top:3px solid #6DCA31; padding:5px; background-color:#F1F1F1;">  		
	  		<h4>Още снимки</h4>			
				<ul id="thumbs_posts">';
	...
	$post_big .= '<div class="detailsDiv" style="float:left; width:650px;margin-bottom:20px; border-top:3px solid #6DCA31; padding:5px; background-color:#F1F1F1;">
