...
{
	$survey .= '<div class="boxRight   main_same_height">
		...
		<div id="SurveyDiv" align="left" style="font-size:12px; font-family:\'Trebuchet MS\', Arial,sans-serif; padding-left:10px; padding:10px 0px 10px 0px; ">
		  ...
		  <div class="detailsDiv" style="font-size:14px; font-family:\'Trebuchet MS\', Arial,sans-serif; float:left;  margin-bottom:20px; margin-right:5px; border-top:3px solid #0099FF; padding:5px; background-color:#F1F1F1;">
			'.$resultSurveysActiv['survey_body'].'
		</div>
		<br style="clear:left;"/>';
	 
		foreach($resultAnsers as $anser)
		{ 
		       				
			$survey .= '<div style=" "><input type="radio" name="ansers" id="ansers'.$anser['anserID'].'" value="'.$anser['anserID'].'" onclick="if(this.checked==1){$(\'radio\').setValue(this.value);}"> '.$anser['anser'].'	 </div>';
	   	
		}
		
			$survey .= '<div style=" margin-top:20px; " align="center">
			<input type="image" onclick="if($(\'radio\').getValue()==\'\'){alert(\'Моля изберете една от предоставените Ви опции!\');return false;} else{new Ajax.Updater(\'SurveyDiv\', \'includes/tools/Ajax_survey.php\', {
						  method: \'get\',
