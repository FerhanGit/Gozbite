 ...
 */
class APHORISMS extends Page
{
	var $site = NULL;
	var $page_name = 'aphorisms';
	var $aphorism_obj = NULL;
	...
	function APHORISMS(& $site)
	{	
		global $conn;
		$this->site = & $site;	
		require_once CLASSDIR."Aphorism.class.php";
		$this->aphorism_obj	= new Aphorism($conn);
...
function displayMainStuff($params)
{
	...
	{
		$prms = array('page_name' => Pages::getCurrentPageName());		
		$main_section .= Pages::loadModule(Pages::getCurrentPageName().'/aphorisms_main_edit',$prms);	
	}
	else 
	{
		if((!isset($_REQUEST['search']) && !isset($_REQUEST['search_btn'])) && isset($_REQUEST['aphorismID']) && $_REQUEST['aphorismID'] > 0)
		{	
			$prms = array('page_name' => Pages::getCurrentPageName());		
			$main_section .= Pages::loadModule(Pages::getCurrentPageName().'/aphorism_big',$prms);	
		}	
		
	       
		$prms = array('page_name' => Pages::getCurrentPageName());		
		$main_section .= Pages::loadModule(Pages::getCurrentPageName().'/aphorisms_search',$prms);
	...
	{
		$prms = array('page_name' => Pages::getCurrentPageName());		
		$main_section .= Pages::loadModule(Pages::getCurrentPageName().'/aphorisms_listing',$prms);	
	}
...
function displayRightColumn($params)
{
	...
	if(basename($GLOBALS['_SERVER']['SCRIPT_NAME']) == 'edit.php')
	{
		$right_section .= Pages::loadModule(Pages::getCurrentPageName().'/insert_edit_aphorism',$prms);	
	}
	
	$right_section .= Pages::loadModule(Pages::getCurrentPageName().'/insert_aphorism_button',$prms);	
	...
	$right_section .= '<ul class="TwoHalf">';
		...
		$right_section .= Pages::loadModule('archive_aphorisms',$prms);	
	$right_section .='</ul>';    		
	*/
...
{		
	$this->aphorism_obj->whereClause = $params['where_clause'];
	$this->aphorism_obj->orderClause = $params['order_clause'];
	$this->aphorism_obj->limitClause = $params['limit_clause'];
   	   
    return $this->aphorism_obj->load(); // ����� array[$aphorismID] � ����������� ��� FALSE
	...
	{		
		...
		if(isset($_REQUEST['aphorismID']))
		{
			print strip_tags(myTruncate(get_aphorism_nameByAphorismID($_REQUEST['aphorismID']), 100, ' ', ' ... ')).' - Фрази, афоризми, умни мисли, забавни изказвания, лозунги и др';
		}
		else 
		{
	...
	function nextPrevious()
	{
		if(isset($_REQUEST['aphorismID']))
		{
			print 'next_previous(\'aphorisms\','.$_REQUEST['aphorismID'].');';
		}
	}
