...
function displayMainStuff($params)
{
	...
	{
		$prms = array('page_name' => Pages::getCurrentPageName());		
		$main_section .= Pages::loadModule(Pages::getCurrentPageName().'/stuff_main_edit',$prms);	
	}
	else 
	{
		...
		{	
			$prms = array('page_name' => Pages::getCurrentPageName());		
			$main_section .= Pages::loadModule(Pages::getCurrentPageName().'/stuff_big',$prms);	
		}	
...
function displayRightColumn($params)
{
	...
	if(basename($GLOBALS['_SERVER']['SCRIPT_NAME']) == 'edit.php')
	{
		$right_section .= Pages::loadModule(Pages::getCurrentPageName().'/insert_edit_stuff',$prms);	
	}
...
function displayFooter($params)
{
	...
	{		
		...
		{
			print 'Животът е сладък - разгледай страница '.strip_tags(getPageTitle($_REQUEST['get']));
		}
		else 
		{
			print 'Животът е сладък';
		}
